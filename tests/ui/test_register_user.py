...
class TestRegister:
    def test_successful_user_register(
        self,
        page,
        config,
        header,
        ...
        signup_page,
        new_user_data,
        account_creation_confirmation_page,
    ):
        home_page.goto()
    ...
    def test_register_while_checkout(
        ...
        expect(
            ...
            f"{new_user_data["title"]} {new_user_data["address"]["first_name"]} {new_user_data["address"]["last_name"]}"
        )
        expect(
            ...
            f"{new_user_data["address"]["city"]} {new_user_data["address"]["state"]} {new_user_data["address"]["zipcode"]}"
        )
        expect(
            checkout_page.delivery_address_list.locator(checkout_page.country)
        ).to_have_text(f"{new_user_data["address"]["country"]}")
        expect(
            checkout_page.delivery_address_list.locator(checkout_page.phone)
        ).to_have_text(f"{new_user_data["address"]["mobile_number"]}")
