...
class TestAddToCart:
    def test_add_to_cart_two_separate_products(self, home_page, page, config, cookie_modal, header, product_listing_page, cart_modal, basket_page):
        ...
        for item in expected_cart_rows:
            ...
            expect(basket_page.get_nth_product_row_price_locator(row)).to_have_text(price)
            expect(basket_page.get_nth_product_row_quantity_locator(row)).to_have_text(quantity)
            expect(basket_page.get_nth_product_row_total_locator(row)).to_have_text(price)

    def test_add_multiple_quantities_of_the_same_product(self, home_page, page, config, cookie_modal, product_listing_page, product_details_page, cart_modal, basket_page):
        product_quantity = "4"
        ...
        expect(basket_page.get_nth_product_row_quantity_locator(1)).to_have_text(product_quantity)