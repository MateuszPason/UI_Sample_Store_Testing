...
live external site over the network). Covers user registration, login/logout, and
account deletion flows. Built with **Playwright (Python, sync API)** + **pytest**,
...
There is **no `requirements.txt` or `pyproject.toml`** in this repo. Dependencies are
whatever the README lists, and it is incomplete/out of date. If a fresh environment or
`import` errors occur, install what's missing:
```bash
pip install playwright pytest pytest-playwright pytest-base-url pyyaml
...
tests/ui/                # test_*.py, pytest classes named Test<Feature>
utils/
  data_generator.py       # generate_new_user_data() (uuid-based unique email/name),
                           # get_correct_login_data()
...
- [README.md](../README.md) — setup/usage guide (has a couple of gaps: missing
  `pytest-playwright`/`pytest-base-url` from its install command, and no `requirements.txt`
  actually exists despite implying `pip install` is enough).
